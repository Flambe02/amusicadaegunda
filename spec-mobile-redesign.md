...
- **Chargement en deux temps :** la miniature YouTube s'affiche d'abord (élément LCP), puis le lecteur YouTube IFrame API (`youtube-nocookie.com`, `autoplay=1`, `mute=1`, `playsinline=1`, `loop=1`, `playlist=<id>`, `controls=0`) se charge après le premier rendu. Fondu vers la vidéo **environ 4 s après le début de la lecture** (PLAYING), au premier chargement comme après chaque glissement, que le son soit coupé ou non : l'interface de démarrage de YouTube (titre, logo « Shorts ») disparaît vers 4 s, et la miniature couvre la vidéo pendant ce temps. La boucle est gérée par l'application (retour au début juste avant la fin), pas par `loop=1&playlist=<id>`, et un seul lecteur est réutilisé (`loadVideoById`).
