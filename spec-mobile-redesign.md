...
- **Chanson sans Short** (`youtube_music_url` vide ; décision du 2026-09-25) — jamais muette :
  - **avec un `youtube_url`** : le même lecteur du feed, caché, joue la chanson complète (YouTube Music), avec les mêmes règles que les Shorts (son au premier geste, pause au tap, barre de progression manipulable, boucle, glissement vers les autres semaines, son conservé en changeant de chanson). À l'écran : la scène de la Caipivara (boucles idle et dance, projecteur des vidéos, bords fondus), qui danse uniquement quand la musique joue réellement avec le son. Titre, ruban du mois et colonne d'icônes comme pour une vidéo ; l'icône Ouvir est masquée pour cette chanson ;
  - **sans `youtube_url`** : la scène de la Caipivara au repos, avec Letra et História, sans lecture. Jamais d'écran vide ;
  - mouvement réduit : le poster fixe de la Caipivara. Toujours un seul lecteur YouTube dans la page.
...
- **Repli :** si PLAYING n'arrive pas en 3 s (mode économie d'énergie, économie de données), la miniature reste, avec le bouton lecture central et le haut-parleur barré (voir ci-dessous). Jamais d'écran vide.
- **Calque au-dessus de l'iframe :** l'utilisateur ne touche jamais l'interface YouTube. Le premier geste de la visite (tap, glissement, flèche, bouton) active le son (`unMute()`, + `playVideo()` si en repli). Ensuite, tap = pause / lecture. Détail : addendum H.2 et H.10.
- **Son coupé (test iPhone du 2026-09-25, remplace le bouton jaune « Toque para ouvir » puis la pastille « Toque para ativar o som ») :** un grand bouton lecture au centre de la vidéo — cercle blanc translucide ≈ 72 px avec ▶, pas de jaune, `aria-label` « Ouvir com som » —, visible dès l'arrivée et jusqu'au premier geste de la visite (tap n'importe où, glissement), puis plus jamais, sauf si le son est refusé après un geste (iOS, économie d'énergie). Tant que le son est coupé, un haut-parleur barré reste en haut à droite (emplacement de Som) ; son actif, le haut-parleur plein (Silenciar).
...
- **Ligne de karaoké sur la vidéo :** seulement si le karaoké de la musique est publié **et sa synchro vérifiable** (décision de la nuit du 2026-09-26, addendum H.16) : chanson sans Short = la chanson complète, la vidéo même du lecteur karaokê → affichée ; Short = un extrait au décalage inconnu → affichée seulement une fois vérifiée sur téléphone (`FEED_KARAOKE_SHORT_VERIFIED_SLUGS`). Synchro par lecture de `getCurrentTime()` et du timing existant, lu comme le lecteur karaokê (`resolveSongTiming`), en lecture seule. Visible uniquement avec le son actif. Sinon, la zone n'existe pas (pas de placeholder). Le mot chanté se remplit de jaune de gauche à droite ; mouvement réduit : ligne entièrement jaune, sans balayage.
- **Un seul jaune par zone :** son coupé = aucun jaune sur la vidéo ; son actif = ligne de karaoké. Rien d'autre en jaune sur la vidéo.
...
Pas de liste : une scène (sous 768 px ; desktop inchangé). **L'écran tient toujours sur une seule page, sans aucun défilement** (comme le feed), barres de Safari affichées comprises : ordre vertical en-tête, carrousel, mois et titre, premier vers, micro et flèches, puis la barre du bas. Si la hauteur manque : d'abord la ligne du premier vers disparaît, puis la carte rétrécit jusqu'à 120 px, puis (écran très bas) les deux sur-titres disparaissent et les marges se resserrent. Le micro reste toujours entièrement visible. Tout le reste de l'ancienne page disparaît sur mobile (Me surpreenda, bulle « Deixe a sorte escolher », filtres, étiquettes de catégorie, pochettes génériques, recherche).
- **En-tête** : « Karaokê » (30 px, 900), « O palco é seu » en eyebrow ; à droite, une icône télé seule (`aria-label` « Festa na TV ») vers `/festa`.
- **Carrousel 3D horizontal** des chansons au karaokê publié (`isKaraokePublished()`), de la plus récente à la plus ancienne : cartes 9:16 de 150 × 267 px au plus et 120 × 213 px au moins, selon la hauteur réellement disponible, miniature du Short (même repli que la grille de recherche), titre en bas lisible par ombre de texte seulement. Carte centrale pleine taille et nette ; voisines décalées, pivotées en profondeur (perspective, `rotateY` ≈ 38°, `translateZ` négatif), atténuées (55 % puis 25 %) ; au-delà, invisibles. Navigation : glissement horizontal (seuil ≈ 30 px, jamais depuis les 24 px du bord gauche — geste retour d'iOS), tap sur une carte latérale, flèches (boutons et clavier). Transition ≈ 550 ms, courbe douce. Tap sur la carte centrale = Cantar.
...
- [ ] Sous 768 px, la home affiche le Short de la semaine en plein écran, son coupé, sans action de l'utilisateur (ou la miniature + le bouton lecture central si l'autoplay est bloqué).
